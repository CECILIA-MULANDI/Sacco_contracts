...
contract BlockCoopTokens is Ownable, ReentrancyGuard, Pausable {
    ...
    /**
     * @dev Liquidate collateral and transfer to specified recipients
     * @param user The borrower whose collateral is being liquidated
     * @param _tokenAddress The token being liquidated
     * @param _loanId The loan ID
     * @param recipients Array of addresses to receive portions
        ...
        whenNotEmergencyPaused
    {
        ...
        uint256 lockedAmount = collateralAmounts[user][_tokenAddress][_loanId];
        ...
        require(
            totalDistribution <= lockedAmount,
            "Distribution exceeds locked amount"
        );

        // First unlock the collateral
        collateralLocked[user][_tokenAddress][_loanId] = false;
        collateralAmounts[user][_tokenAddress][_loanId] = 0;
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            if (amounts[i] > 0) {
                IERC20(_tokenAddress).safeTransfer(recipients[i], amounts[i]);
            }
        }
        ...
        )
    {
        ...
        uint256 end = _offset + _limit > tokenList.length
            ? tokenList.length
            : _offset + _limit;
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            if (!whiteListedTokens[tokenAddress].isWhitelisted) {
                continue; // Skip early to save gas
            }
                ...
                if (priceFeed != address(0)) {
                    ...
                    returns (
                        ...
                        if (
                            ...
                            // Cache decimals to avoid repeated calls
    ...
    function approveLoanManager(
        ...
        IERC20(_token).approve(address(loanManager), _amount);
        emit SpenderApproved(address(loanManager), _token, _amount);
    }
